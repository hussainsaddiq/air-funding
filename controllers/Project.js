...
module.exports.create = (req, res) => {    
        res.render("users/create", {title: 'profile', auth: req.cookies.donationUser, user: req.user, errors: [], fields: {}});
}
        ...
        form.parse(req, async (err, fields, files) => {
                if (!err) {
                       console.log(fields)
                        ...
                        if (fields.amount.trim().length === 0 || parseInt(fields.amount) < 30000) {
                        ...
                        if(fields.end_date.trim().length === 0) {
                                ...
                                errors.push({name:'end_date' ,msg: 'end date should be above than start date'})
                ...
                images[`image${i + 1}`] = imageName;
                              if(i === 0) {
                                if (
                                        ...
                                        errors.push({name: 'doc_1', msg: 'only jpeg/jpg and png image is required'})
                                        // return;
                                      } else {
                      ...
                      if(errors.length === 0) {
                         const {title, amount, start_date, end_date, category, description} = fields;
                         const user = req.user;
                        try {
...
module.exports.projects = async (req, res) => {
        ...
        try {
                ...
                res.render('users/projects',{title: 'projects', auth: req.cookies.donationUser, user: req.user, projects})
...
module.exports.getProject = async (req, res) => {
        ...
        try {
                ...
                console.log(project); 
                res.render('users/details',{title: project.title, auth: req.cookies.donationUser, user: req.user, project, moment})   
        } catch (err) {
