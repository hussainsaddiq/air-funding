...
module.exports.payment = async (req, res, next) => {
  ...
  const donationResponse = await DonationModel.create({product: Types.ObjectId(id),donation: amount}) 
  console.log('donation response', donationResponse);
        const ress = await ProjectModel.findOneAndUpdate({_id: id},{ $push: { donations: donationResponse._id } })
...
module.exports.projects = async (req, res) => {
  ...
  try {
    const projects = await ProjectModel.find().where('status').equals(1);
    res.render('home', {title: 'air funding', auth: req.cookies.donationUser, projects })
...
module.exports.project = async (req, res) => {
  ...
  try {
      ...
      res.render('project',{title: project.title, auth: false,project})
  } catch (error) {
      console.log(error.message);
  }
}
module.exports.checkOut = async (request, response) => {
  const endpointSecret = "whsec_63561ea438fa589a9de6047e5abcae4bdb9a30a0d611804420e18a446611b2f9";
